...
2. 메모리 논리적 주소, 물리적 공간의 단편화 방지
   (메모리풀 내부적으로 부득이한 동적할당은 별도의 오브젝트풀을 구현해 사용합니다)
3. 부가적인 메모리 소모비용의 최소화
4. 속도
5. 범용성을 위해 사용하지 않는 메모리는 가능한 해제를 시도
   (보통 개인이 메모리풀을 만드는 경우는 프로그램 종료시까지 예약주소, 물리적메모리를 반납하지 않는 경우가 많습니다)
...
그안에 V2 폴더에 있는 것이 최근 버전입니다(2015년까지 작업)
...
(물론 정신나간 대기는 하지 않고 스힌 횟수에 제한을 두어 스레드스위칭은 시도합니다)
(수정하고 싶다면 잠금이 사용된 부분은 사실 많지 않으니 쉽게 하실 수 있습니다)


이 메모리풀은 게임같은 곳에 사용시 LFH, TBB에 비해 좀더 메모리를 절약합니다
(아마두요)
(계속 해서 LFH, TBB 와 비교해왔거든요)
LFH에 비해 좀더 다양한 풀로 분할했습니다
청크 단위는 256KB이며(아마 제 기억이 맞을 겁니다)
실제 virtualalloc 시에는 256KB * x 단위로 한번에 더 많이 할당합니다
...
### 이 메모리풀은 8논리코어에 최적화 되어있습니다(사실 혼자 쓰려 했는데 라이젠때문에 메인스트림이 12, 16 스레드로 바뀌어서 오픈소스화 합니다)
